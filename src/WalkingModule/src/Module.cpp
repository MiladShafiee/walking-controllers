...
bool WalkingModule::configure(yarp::os::ResourceFinder& rf)
{
    ...
    if(m_useStepAdaptation)
    {
        ...
        if(!m_stepAdapter->initialize(stepAdaptatorOptions))
        {
    ...
    //initialize the step adaptation Utils
...
jointsError=jointsError+0.1;
        ...
        if(m_useStepAdaptation){
            ...
            if (!m_leftInContact.front() || !m_rightInContact.front())
            {
                ...
                if(!m_stepAdapter->solve(!m_leftInContact.front()))
                {
                ...
                if (!m_leftInContact.front())
                {
                    ...
                    if(!m_stepAdapter->getAdaptatedFootTrajectory(m_stepHeight, m_dT, firstSS->getTrajectoryDomain().first,
